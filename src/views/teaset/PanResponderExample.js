...
export default class PanResponderExample extends NavigationPage {
  ...
  handleTouches(touches) {
    ...
    if (distance0 && distance1) {
      const scaleRate = distance1 / distance0
      const scalePointX = (prevTouches[1].pageX + prevTouches[0].pageX) / 2
      const scalePointY = (prevTouches[1].pageY + prevTouches[0].pageY) / 2
      this.refs['view'].measure((x, y, width, height, pageX, pageY) => {
        // view center point position
        const viewCenterX = x + width / 2
        const viewCenterY = y + height / 2
        // the scale point with the center of the view as the origin
        const spBeforScaleX = scalePointX - viewCenterX
        const spBeforScaleY = scalePointY - viewCenterY
        const spAfterScaleX = spBeforScaleX * scaleRate
        const spAfterScaleY = spBeforScaleY * scaleRate
      ...
      <View style={{flex: 1}}>
        ...
        <View style={{backgroundColor: '#fff', padding: 10, overflow: 'hidden'}}>
          ...
          >
            ...
            <Image resizeMode='cover' source={require('../../assets/images/teaset1.jpg')} style={{width: 300, height: 500}} />
            <TouchableOpacity onPress={() => console.log('press')} style={{flex: 1}} />
          </View>
        </View>
      </View>
    )
...
class MyTouchableOpacity extends TouchableOpacity {
  constructor(props) {
    ...
    const touchableHandleStartShouldSetResponder = this.touchableHandleStartShouldSetResponder
    const touchableHandleResponderTerminationRequest = this.touchableHandleResponderTerminationRequest
    const touchableHandleResponderGrant = this.touchableHandleResponderGrant
    const touchableHandleResponderMove = this.touchableHandleResponderMove
    const touchableHandleResponderRelease = this.touchableHandleResponderRelease
    const touchableHandleResponderTerminate = this.touchableHandleResponderTerminate
